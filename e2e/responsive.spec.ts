...
import { BREAKPOINTS, ROUTES } from "./routes";

for (const route of ROUTES) {
  for (const width of BREAKPOINTS) {
    test(`RESP-1: ${route.name} does not overflow horizontally at ${width}px`, async ({
      page,
    }) => {
...
test("LAND-3: the header Sign in control appears only past the sm breakpoint", async ({
  page,
}) => {
  await page.goto("/");
