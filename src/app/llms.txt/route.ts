...
- One countdown runs for the whole interview, not per question, and it doesn't pause between questions. Each question's time starts once it has been asked: the moment it is on screen when typing, or, when answering aloud, once the browser has read it out (the reading can be skipped, and the microphone stays off during it). The first question is asked when "Go" is pressed, and each later one the moment the previous answer is submitted. Closing the tab stops the clock; returning picks up the same questions with the same time left.
