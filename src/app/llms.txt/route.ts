...
- One countdown runs for the whole interview, not per question, and it doesn't pause between questions. Each question's time starts once it has been asked: once the browser has read it out, or straight away where it can't (the reading can be skipped, and the microphone stays off during it). If the microphone fails mid-answer, the clock stops until it is back. The first question is asked when "Go" is pressed, and each later one the moment the previous answer is submitted. Closing the tab stops the clock; returning picks up the same questions with the same time left.
- Answers are spoken aloud, transcribed by the browser. No audio is recorded, uploaded, or stored — only the text. A browser that cannot transcribe speech cannot start a run.
