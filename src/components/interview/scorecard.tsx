 ...
 * as stars and a band word, never a bare number (interview second pass ticket 02). A question the
 * countdown ran out before reads "Not reached", with nothing to score, and so does a Category that
 * had nothing reached (ticket 03).
 *
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="max-w-prose text-sm text-muted-foreground">
          ...
          {unreached > 0 && `, ${unreached} of which you didn’t get to`}. The lowest area below is the
        ...
        return (
          <section key={category} aria-labelledby={`scorecard-${category}`} className="space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1 border-b border-border pb-1">
              ...
              ) : rollup.unreached === rollup.questions ? (
                <NotReached />
              ) : (
...
/** Where stars would be, for a question — or a whole Category — the countdown ran out before. */
function NotReached() {
