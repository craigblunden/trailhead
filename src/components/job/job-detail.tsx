 ...
 * ticket 07). Beside the cover letter's card above, because the two are the same kind of thing:
 * what this application does with a Job once the posting and the resume are on it.
 *
 * Shown on every Plan and at every Stage. A Tenant not on `pro` lands on the locked start screen,
 * which is the point of the preview; and a Job at `interviewing` is exactly the one worth
 * rehearsing for, so nothing here reads the Stage.
 */
function InterviewJumpCard({ job }: { job: Job }) {
  return (
    <Card className="bg-accent/70 ring-primary/15 [--card-spacing:--spacing(4)]">
      <CardContent className="flex flex-col">
        <p className="text-sm">
          Rehearse for this interview against questions written from this posting and your resume.
        </p>
